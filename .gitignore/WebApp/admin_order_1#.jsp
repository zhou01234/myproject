...
<div id="main">
	...
	<hr>
	<form id="query" name="query" method="post" action="ListRequest">
	<table><tr>
	    ...
	    <table align='center' style="font-size:20px;text-align:center;" border="1" >
	    	...
	    	<c:forEach items="${LIST}" var="r" varStatus="status">
	    		<tr id="r_${status.index}">
					...
					<td><input class='qf' id='r_${status.index}_3' disabled="disabled" value="${r.longterm}"></td>
	    			<td><input class='qf' id='r_${status.index}_4' disabled="disabled" value="${r.building}"></td>
					<td><input class='qf' id='r_${status.index}_5' disabled="disabled" value="${r.roomNum}"></td>
					<td><input class='qf' id='r_${status.index}_6' disabled="disabled" value="${r.username}"></td>
					
					<input id='r_${status.index}_7' style="display:none" value="${r.name}">
					<input id='r_${status.index}_8' style="display:none" value="${r.email}">
					<input id='r_${status.index}_9' style="display:none" value="${r.reason}">
					
					<td><a href="javascript:alert('${r.reason}')">查看</a></td>					
					<td>
						<input type="button" onclick="pass(${status.index});" value="通过"/>&nbsp;
...
<script type="text/javascript">
 ...
 function pass(id) {
	 ...
	 var longterm=document.getElementById("r_"+id+"_3").value;
	 var building=document.getElementById("r_"+id+"_4").value;
	 var roomNum=document.getElementById("r_"+id+"_5").value;
	 var username=document.getElementById("r_"+id+"_6").value;
	 var name=document.getElementById("r_"+id+"_7").value;
	 var email=document.getElementById("r_"+id+"_8").value;
	 var reason=document.getElementById("r_"+id+"_9").value;
	 
	 var Form=document.createElement("form");
	 document.body.appendChild(Form);
	 Form.name="Form";
	 Form.method = "post";
	 Form.action = "PassRequest";
